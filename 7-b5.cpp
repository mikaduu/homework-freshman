...
private:
	...
	char round1[MAX_NAME_LEN];//第一轮的标识
	char round2[MAX_NAME_LEN];//第二轮的标识
...
{
	int i,j,chongfu,k,m;
	for (k=0,i = 0; i<list_num_1; i++)//循环合并第一个文件，剔除重复
	{
		chongfu = 0;
		for (j = 0; j < k; j++)
		{
	...
	for (i = 0; i<list_num_2 && k < MAX_STU_NUM; i++)//循环合并第二个文件，剔除重复(要考虑在第二个文件中本身重复的情况)
	{
		chongfu = 0;
		for (m = 0; m < i; m++)
		{
		...
		if (chongfu == 0)
		{
			...
			strcpy(list_merge[k].round2, "补选");
			k++;
		}
	}
	list_merge_num = k;
	for (i = 0; i < list_merge_num; i++)//补充第二轮退课
	{
		chongfu = 0;
