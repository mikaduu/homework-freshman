...
void calc_num(char pure_decimal[], char out[], char power_table[][N_NUM + 1])
{
	...
	for (i = 1; i < N_NUM; i++)
	{
		for (m = 1; m < N_NUM; m++)
		{
			out[m] += int(pure_decimal[i]) * int(power_table[i][m]);
		}
	}
	for (i = N_NUM; i > 0; i--)
	{
		if (int(out[i]) >= 10)
		{
			out[i-1] = int(out[i-1])+(int(out[i]) / 10);
			out[i] = int(out[i]) % 10;
		}
	}
	for (i = N_NUM; i > 0; i--)
...
void input_pure_decimal(char pure_decimal[])
{
	...
	while (1)
	{
		...
		for (i = 1; i < N_NUM && pure_decimal[i]!=0; i++)
		{
			if (pure_decimal[i] == '0')
			{
				pure_decimal[i] = 0;
